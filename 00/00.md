...
js的面向对象于Java,C++这样的面向对象不一样，不知道从何讲起
...
创建对象最简单的方式就是再JavaScript代码中使用对象直接量
...
new运算符创建并初始化一个新对象。关键字new后跟随一个函数调用，这里的函数称作构造函数，构造函数用以初始化一个新创建的对象。JavaScript语言核心里中的原始类型都包含内置的构造函数。例如：

```javascript
const o = new Object();		//创建一个空对象，和{}一样
const a = new Array();		//创建一个空数组，和[]一样
const d = new Date();		//创建一个表示当前时间的Date对象
const r = new RegExp("js");		//创建一个可以进行模式匹配的RegExp对象
```



原型，每一个JavaScript都和一个圆形对象相关联

并可以通过Object.prototype获得对原型对象的引用



Object.create()是一个静态函数，而不是提供给某个对象的调用方法，使用它的方法很简单。只需要传入所需的原型对象即可。
